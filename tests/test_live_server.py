...
def test_the_panel_answers_on_ipv4_and_ipv6(live):
    """The platform edge picks the address family, not us.

    A panel that answers on 127.0.0.1 but refuses `::1` looks perfectly healthy
    from the inside and is a 502 from outside, so both are checked against a real
    server rather than a TestClient.
    """
    import urllib.parse
